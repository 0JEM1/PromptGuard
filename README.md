...
Paste a risky prompt (AWS key + PII)
Run scan and show score + issue categories
Copy redacted output
Open Dashboard and show locally persisted violations
Open Policies and add an organization-specific custom rule
Explain OpenClaw contextual detection advantage
GitHub Upload Checklist
Verify env values are local-safe (no secrets committed)
Confirm app runs: npm run dev
Confirm extension loads from extension/manifest.json
Commit and push:
git add .
